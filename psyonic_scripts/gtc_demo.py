...
class PsyonicURSceneCfg(InteractiveSceneCfg):
    ...
    robot: ArticulationCfg = ArticulationCfg(
        ...
        # initializes actuators of Robot
        actuators={
...
class PsyonicNode(Node):
    ...
    def __init__(self):
        ...
        self.subscriber = self.create_subscription(JointState, 'IsaacLab_Topic', self.sim_callback, qos_profile_best_effort)
    ...
    def sim_callback(self, msg):
        self.msg = list(msg.position)

        self.msg[8], self.msg[9] = self.msg[9], self.msg[8]
        self.msg[10], self.msg[11] = self.msg[11], self.msg[10]

        # offsets for arm position if needed
        self.msg[0] += 1.5708 #####ADD ARM OFFSETS HERE!!!
        # self.msg[1] += 0.0 #####ADD ARM OFFSETS HERE!!!
        # self.msg[2] += 0.0 #####ADD ARM OFFSETS HERE!!!
        # self.msg[3] += 0.0 #####ADD ARM OFFSETS HERE!!!
        # self.msg[4] += 0.0 #####ADD ARM OFFSETS HERE!!!
        # self.msg[5] += 0.0 #####ADD ARM OFFSETS HERE!!!
        
        # logs msg for debugging purposes
        self.get_logger().info(f'recieved: {self.msg}')
        
        # transfers msg into a tensor
        action = torch.tensor(self.msg, device=self.env.device)
